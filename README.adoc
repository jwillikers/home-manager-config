...
:Private-nixpkgs: https://forgejo.jwillikers.io/jwillikers/private-nixpkgs/[Private nixpkgs]
...
StartupNotify=false
----
.. Disable Steam from starting automatically in desktop mode by deleting the Steam desktop file from the autostart directory.
+
[,sh]
----
sudo rm /etc/xdg/autostart/steam.desktop
----
...
For the Steam Deck, I created the `deck` user account for this purpose and shared the {Nix-Secrets} and {Private-nixpkgs} repositories with this user, giving it read-only access in the repositories' settings.
...
Run `nvidia-smi --version` or `dnf list --installed \*nvidia\*` to show the installed driver version.
...
This command must be run after every update of the service.

[,sh]
----
sudo restorecon -v /nix/store/*-non-nixos-gpu/lib/systemd/system/non-nixos-gpu.service
----
...
. After installation, open Lutris and log in to GOG so that comet can connect.
