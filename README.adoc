...
.. Optionally, open a terminal and configure the Steam Deck to use Plasma Wayland as the default session.
...
// Alternatively, use the `plasma-wayland-persistent` argument to boot into the Plasma Wayland desktop session, skipping Gaming Mode.
// The following command demonstrates how to enable the former.
+
[,sh]
----
